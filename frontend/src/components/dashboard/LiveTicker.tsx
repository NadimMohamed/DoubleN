...
export function LiveTicker({ symbol, initialPrice, initialChangePct, showDetails = false }: LiveTickerProps) {
  ...
  return (
    <div className="flex flex-col gap-1">
      <div className="flex items-center gap-2">
        {/* Live indicator */}
        <div className={cn('w-1.5 h-1.5 rounded-full flex-shrink-0', isConnected ? 'bg-emerald animate-pulse' : 'bg-slate')} />
      ...
      {showDetails && (
        <div className="flex items-center gap-4 text-xs text-slate">
          ...
          {volume && <span>Vol: <span className="text-white">{volume.toLocaleString('en-US', { maximumFractionDigits: 0 })}</span></span>}
        </div>
      )}
    </div>
  )
